...
function inferGithubRepo() {
  ...
  if (host.endsWith(".github.io")) {
    ...
    const repo = parts[0] || "app14_weather_markets";
    return { owner, repo };
  }
...
async function load() {
  try {
    ...
    els.updated.textContent = `Updated ${generatedAt} · ${rows.length} rows (${noCount} Buy No) · ${stats.eventsScored ?? "—"} events · auto every ~5 min`;
    render();
    return data;
  } catch (err) {
    console.error(err);
